...
def test_remove_tab_dialog(app_qt_fixture, ui_no_refs_config_fixture, mocker):
    ...
    mocker.patch.object(
        QtWidgets.QMessageBox, "exec", return_value=QtWidgets.QMessageBox.StandardButton.No
    )
...
def test_edit_AppLink(app_qt_fixture, base_fixture, ui_config_fixture, mocker):
    ...
    assert (
        config_tabs[0].name == tab_model.name == "Basics"
    )  # just safety that it is the same tab
    ...
    app_config = UiAppLinkConfig(name="NewApp", conan_ref=TEST_REF, executable="bin/exe")
...
def test_add_AppLink(app_qt_fixture, base_fixture, ui_no_refs_config_fixture, mocker):
    ...
    app.conan_api.get_path_with_auto_install(ConanRef.loads(TEST_REF), {})
    ...
    app_config = UiAppLinkConfig(
        name="NewApp", conan_ref=TEST_REF, executable="conanmanifest.txt"
    )
    app_model = UiAppLinkModel().load(app_config, app_link.model.parent)
...
def test_move_AppLink(app_qt_fixture, base_fixture, ui_no_refs_config_fixture, mocker):
    ...
    move_dialog._ui.list_view.selectionModel().select(
        sel_idx, QtCore.QItemSelectionModel.SelectionFlag.Select
    )
...
def test_multiple_apps_ungreying(app_qt_fixture, base_fixture):
    ...
    config_file_path = (
        base_fixture.testdata_path / "config_file/multiple_apps_same_package.json"
    )
