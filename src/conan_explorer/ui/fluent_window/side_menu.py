...
class SideSubMenu(ThemedWidget):
    def __init__(
        self, parent_stacked_widget: QStackedWidget, title: str = "", is_top_level=False
    ):
    ...
    def add_named_custom_entry(
        ...
        frame = QFrame(self)
        if force_v_layout or label.width() > (
            RIGHT_MENU_MAX_WIDTH - widget.width() - 30
        ):  # aggressive 30 px padding
    ...
    def add_toggle_menu_entry(
        self, name: str, target: Callable, initial_state: bool, asset_icon: str = ""
    ):
