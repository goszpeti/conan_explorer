...
class EditableEditDialog(QDialog):
    def __init__(
        self,
        editable: Optional[EditablePkg],
        editable_controller: ConanEditableController,
        ...
        self._ui.output_folder_browse_button.clicked.connect(
            self.on_output_browse_button_clicked
        )
    ...
    def save(self):
        ...
        new_editable = EditablePkg(
            new_name,
            self._ui.path_line_edit.text().strip(),
            self._ui.output_folder_line_edit.text(),
        )
        if self._editable_controller.add(new_editable) and remove_after_add:
