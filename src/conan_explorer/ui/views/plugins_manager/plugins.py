...
from conan_explorer import AUTHOR, BUILT_IN_PLUGIN, DEBUG_LEVEL, base_path
...
class PluginsPage(PluginInterfaceV1):
    ...
    def __init__(
        ...
        self._ui.plugins_tree_view.selectionModel().selectionChanged.connect(
    ...
    def on_plugin_selected(self):
        ...
        if plugin.data(1) == BUILT_IN_PLUGIN:
            self.set_themed_icon(
                self._ui.remove_plugin_button, "icons/delete.svg", force_light_mode=True
            )
    ...
    def on_add(self):
        ...
        dialog = QFileDialog(
            parent=self,
            caption="Select Plugin description file",
    ...
    def on_remove(self):
        ...
        message_box.setText(
            f"Are you sure, you want to remove the plugin {selected_item.data(0)}?"
        )
