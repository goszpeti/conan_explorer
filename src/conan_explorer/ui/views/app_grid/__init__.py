...
class AppGridView(PluginInterfaceV1):
    ...
    def _init_right_menu(self):
        ...
        quicklaunch_submenu.add_toggle_menu_entry(
            "Auto-install quicklaunch packages",
            self.on_toggle_auto_install,
            app.active_settings.get_bool(AUTO_INSTALL_QUICKLAUNCH_REFS),
        )
    ...
    def on_new_tab(self):
        ...
        if accepted:
            ...
            if not text:
                return
            # update model
            tab_model = UiTabModel().load(
                UiTabConfig(text, apps=[UiAppLinkConfig()]), self.model
            )
    ...
    def open_config_file_dialog(self):
        ...
        dialog = QFileDialog(
            parent=self,
            caption="Select JSON Config File",
            directory=str(dialog_path),
                    ...
                    if app_link.model.conan_ref == conan_ref:
                        ...
                        if (
                            ...
                            if (
                                ...
                                continue
                        if pkg_id:
                            app_link.model.set_package_folder(
                                app.conan_api.get_package_folder(
