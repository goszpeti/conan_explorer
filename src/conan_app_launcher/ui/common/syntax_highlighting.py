...
class ConfigHighlighter(QSyntaxHighlighter):
    ...
    def __init__(self, parent, type: Union[Literal["ini"], Literal["yaml"]]) -> None:
        ...
        if app.active_settings.get(GUI_MODE) == GUI_MODE_DARK:
            self.key_color = (155, 220, 254)
            self.value_color = (206, 145, 120)
            self.section_color = (80, 201, 177)
        else: 
            self.section_color = (37, 127, 153)
            self.key_color = (0, 30, 253)
            self.value_color = (172, 22, 19)
        self.comment_color = (107, 152, 87)
        if type == "ini":
            self._separator = "="
            self._key_regex = "(.*?)(?=\=)"
            self._value_regex = "(?<=\=)(.*?)$" # ((.|\n)*)(?=\=)            
